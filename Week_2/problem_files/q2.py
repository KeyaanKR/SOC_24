...
class History:
    ...
    def get_valid_actions(self):
        valid_actions = []
        # Go through all the boards and get the valid actions
    ...
    def get_value_given_terminal_history(self):
        ...
        if self.is_terminal_history():
            return 1 if self.is_win == 1 else -1
        return 0
...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    # check if the history is terminal
    ...
    if max_player_flag:
        ...
        for action in valid_actions:
            new_history = History(history_obj.num_boards, history_obj.history + [action])
...
def maxmin(history_obj, max_player_flag):
    ...
    # Check if the board has been evaluated
